...
class Nosto_Tagging_Block_Adminhtml_System_Config_Currency_Method extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
        ...
        if ($store instanceof Mage_Core_Model_Store) {
            /* @var Nosto_Tagging_Helper_Account*/
            $accountHelper = Mage::helper('nosto_tagging/account');
            /* @var NostoAccount $nostoAccount */
            $nostoAccount = $accountHelper->find($store);
            if($nostoAccount instanceof NostoAccountMetaInterface) {
                $missingScopes = $nostoAccount->getMissingScopes();
                $requiredScopes = array(
                    NostoApiToken::API_SETTINGS,
                    NostoApiToken::API_EXCHANGE_RATES
                );
                if (is_array($missingScopes)) {
                    foreach ($missingScopes as $missingScope) {
                        if (in_array($missingScope, $requiredScopes)) {
                            $disabled = true;
                            break;
                        }
                    }
